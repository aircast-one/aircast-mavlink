...
describe('Performance and Stress Tests', () => {
  ...
  describe('Serialization Performance', () => {
    test('should serialize HEARTBEAT messages at high throughput', () => {
      ...
      for (let i = 0; i < iterations; i++) {
        serializer.serialize('HEARTBEAT', payload, {
          system_id: 1,
          component_id: 1,
          sequence: i % 256,
        })
    ...
    test('should serialize complex SYS_STATUS messages efficiently', () => {
      ...
      for (let i = 0; i < iterations; i++) {
        serializer.serialize(
          ...
          { system_id: 1, component_id: 1, sequence: i % 256 }
        )
      }
  ...
  describe('Parsing Performance', () => {
    ...
    test('should handle large message buffers efficiently', () => {
      ...
      for (let i = 0; i < messageCount; i++) {
        const bytes = serializer.serialize('HEARTBEAT', payload, {
          system_id: 1,
          component_id: 1,
          sequence: i % 256,
        })
  ...
  describe('Edge Case Stress Tests', () => {
    test('should handle maximum field values without overflow', () => {
      ...
      for (let i = 0; i < iterations; i++) {
        const bytes = serializer.serialize('HEARTBEAT', maxPayload, {
          system_id: 255,
          component_id: 255,
          sequence: 255,
        })
    ...
    test('should handle rapid buffer operations', () => {
      ...
      const serializedMessage = serializer.serialize('HEARTBEAT', payload, {
        system_id: 1,
        component_id: 1,
        sequence: 0,
      })
    ...
    test('should handle concurrent serialization with different dialects', () => {
      ...
      const options = { system_id: 1, component_id: 1, sequence: 0 }

      const iterations = 1000

      for (let i = 0; i < iterations; i++) {
        // Serialize same message with both dialects
        const commonBytes = serializer.serialize('HEARTBEAT', payload, options)
        const minimalBytes = minimalSerializer.serialize('HEARTBEAT', payload, options)
