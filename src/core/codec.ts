 ...
 */
export function encodeSingleValue(view: DataView, offset: number, type: string, value: unknown): number {
 ...
 */
export function encodeField(view: DataView, offset: number, field: FieldDefinition, value: unknown): number {
  const isArray = field.arrayLength !== undefined;
  const arrayLength = field.arrayLength || 1;
 ...
 */
export function encodePayload(message: Record<string, unknown>, fields: FieldDefinition[]): Uint8Array {
  const sortedFields = sortFieldsByWireOrder(fields);
